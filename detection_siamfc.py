# -*- coding: utf-8 -*-
"""[최종]detection_siamfc.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tVa5gcWXlhSJYueauefIS_Zq7R8r-lQx
...
# sys.path.append('/content/drive/MyDrive/Colab Notebooks/CVIP 랩/UAV/Track 1')
...
from yolov5.utils.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2, increment_path, non_max_suppression, print_args, scale_boxes, strip_optimizer, xyxy2xywh)
...
class SiamFC(nn.Module):
    def __init__(self):
        ...
        self.feature = nn.Sequential(
            ...
            # 일반적으로 eps = 0.5, momentum=0.9를 사용하는데, 어떠한 계산으로 이러한 파라미터 조정이..
            ...
            # Batch별 독립 correlation 수행 : 입력과 출력을 나눔 [입력 4채널 -> (2,2) 출력 8채널 -> (4,4)]
            nn.BatchNorm2d(256, eps=1e-6, momentum=0.05), # Batch 안의 각 샘플을 서로 섞지 않기 위해서 groups 사용
            nn.ReLU(inplace=True),
            nn.MaxPool2d(3, 2),
...
class TrackerSiamFC(object):
    ...
    def parse_args(self, **kargs):
        cfg={
            ...
            'hide_labels': False,
            'hide_conf': False,
            #'weight': 'yolov5/runs/train/exp/weights/best.pt',# pt
            'weight' : '/content/drive/MyDrive/Colab Notebooks/CVIP_LAB/UAV/Track 1/yolov5/runs/train/exp/weights/best.pt',
    ...
    def init(self, image, model):
        ...
        imgsz = check_img_size(self.cfg.imgsz, s=stride)

        im = letterbox(image, imgsz, stride=stride, auto=True)[0]
        ...
        if len(det) == 0:
            ...
            exemplar_image = torch.from_numpy(exemplar_image).to(self.device).float()
            exemplar_image = exemplar_image.permute([2, 0, 1]).unsqueeze(0)
    ...
    def init_with_bbox(self, image, bbox):
        image = np.asarray(image)
        target_bbox = bbox
        ...
        # exemplar features
        exemplar_image = torch.from_numpy(exemplar_image).to(
            self.device).permute([2, 0, 1]).unsqueeze_(0).float()
        with torch.set_grad_enabled(False):
            self.net.eval()
            self.kernel=self.net.feature(exemplar_image)
        ...
        cv2.rectangle(im_disp, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(im_disp, "GT Init", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    def update(self, image):
        ...
        instance_images = torch.from_numpy(instance_images).to(self.device).float()
        instance_images = instance_images.permute([0, 3, 1, 2])
        ...
        with torch.set_grad_enabled(False):
            ...
            if self.kernel.device != instances.device:
        ...
        # # Confidence Score Thresholding 적용하기
        # current_score = response.max()
        # conf_threshold = 0.15
